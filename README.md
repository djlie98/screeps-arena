...
common/                    <- code shared across arenas (see below)
  tsconfig.json            <- lets the IDE resolve common/*.ts in isolation (see below)
  typings/                 <- generated by consolidate-typings.mjs, gitignored - do not edit
scripts/
  arenas.mjs               <- discovers arenas/typings sources
  patch-typings.mjs        <- fixes a handful of bugs in the game's generated .d.ts files (see below)
  consolidate-typings.mjs  <- merges every arena's typings/ into common/typings/ (see below)
...
### Where `common/`'s `"game/*"` types come from

`common/*.ts` files (like `rush.ts`) import from `"game/utils"` etc. just like arena code does, but `common/` isn't an arena - it has no `typings/` of its own. Every arena's own `tsconfig.json` still type-checks `common/` correctly (each arena's build pulls `common/rush.ts` into its own program and resolves `"game/*"` against *that arena's* `typings/`), but that leaves nothing for the IDE to use when you open a file under `common/` directly - there's no tsconfig.json to walk up to.

`scripts/consolidate-typings.mjs` fixes this by merging every arena's `typings/game/` into `common/typings/game/` (one canonical file per path - if two arenas ever actually disagree, it prints a warning naming which ones and uses whichever version most arenas agree on, rather than silently picking one) plus each arena's non-`game/` typings namespaced under `common/typings/arena-specific/<arena>/`. `common/tsconfig.json` points at this consolidated copy, so opening any file under `common/` on its own resolves correctly.

`common/typings/` is gitignored - it's fully regenerated (deleted and rebuilt from scratch) by `npm run consolidate-typings`, which also runs automatically as part of `npm run build`/`typecheck` (via a shared `sync-typings` step, alongside `patch-typings`) and at the end of `npm run add-arena`. You shouldn't normally need to run it by hand.
...
`patch-typings.mjs` fixes them in place, and is idempotent (safe to run repeatedly, a no-op once already applied). It runs automatically before `build` and `typecheck` (via the shared `sync-typings` step behind the `prebuild`/`pretypecheck`/`prewatch` npm hooks - see below), so if the game re-syncs fresh typings and reintroduces the bugs, they're fixed again on your next build.
